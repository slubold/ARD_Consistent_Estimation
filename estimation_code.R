...
z_0 = c(0, 0) # location of the reference node that we want to estimate.
numSim = 25 # number of simulations per sample size.
n.vec = c(50, 100, 500, 1000, 10^4) # various sample sizes used for estimation.
...
# Run this code to plot and save the estimation results.
pdf("compare_n_z_i_scatter.pdf")
par(mfrow=c(1,2))    # set the plotting area into a 1*2 array
plot(z.hat.saved[1,,1], z.hat.saved[2,,1], pch = 19, xlab = "", ylab = "", main=TeX('Estimate of node location $\\z_i$ with n = 50'))
plot(z.hat.saved[1,,5], z.hat.saved[2,,5], pch = 19,  xlab = "", ylab = "", main=TeX('Estimate of node location $\\z_i$ with n = 10^4'))
dev.off()
...
# Run this code to plot and save the node effect estimates.
pdf("node_effects.pdf")
boxplot(nu.estimates, names = n.vec, cex.lab = 1.5, cex.axis = 1.25,  lab = "n", ylab = "Estimate of node effect", main=TeX('Estimate of node effect $\\nu_i$'))
